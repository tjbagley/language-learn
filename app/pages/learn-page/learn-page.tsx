...
export default function LearnPage() {
  ...
  const whatToLearnListId = useSelector((state: RootState) =>
    selectWhatToLearnListId(state),
  );
  ...
  const randomiseList = (
    words: Array<WordOrPhrase | null | undefined>,
  ): WordOrPhrase[] => {
    return words.filter((w) => !!w).sort(() => Math.random() - 0.5);
  };
  const randomisedChoices = randomiseList([
    learn?.wordToLearn,
    learn?.choice1,
    learn?.choice2,
  ]);
  ...
  const loadNextWord = (listId: string) => {
    ...
    const wordList = words?.filter((w) =>
      list?.items?.some((i) => i.wordOrPhraseId === w.id),
    );
  ...
  const handleWhatToLearnListChange = (
  ...
  const renderWhatToLearnSelection = (): React.ReactNode => {
    return (
      <div className="learn__selection">
        <label>Learn</label>
        <select
          defaultValue={whatToLearnListId}
          onChange={(e) => handleWhatToLearnListChange(e)}
        >
      ...
      <section className="centered-container">
        <div className="learn">
          ...
          <div className="learn__word">
            ...
            <div className="learn__sounds-like">
              {learn?.wordToLearn?.soundsLike}
            </div>
          </div>
  ...
  return (
    <section className="centered-container">
      <div className="learn">
        {renderWhatToLearnSelection()}
        <div className="learn__word">
          <h1 ref={wordRef} tabIndex={-1}>
            {learn?.wordToLearn?.value}
          </h1>
          <div className="learn__sounds-like">
            {learn?.wordToLearn?.soundsLike}
          </div>
        ...
        <div className="learn__choices">
          {randomisedChoices.map((item, index) => (
            <button
              key={index}
