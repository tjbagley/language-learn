...
const initialState: WordsAndPhrasesState = localStorageData
  ? {
      values: localStorageData.language?.wordsOrPhrases ?? [],
      recentlyLearntWordsAndPhrases:
        localStorageData.language?.learn?.recentWordsAndPhrases ?? [],
      whatToLearnListId:
        localStorageData.language?.learn?.whatToLearnListId
    }
  : {
      values: [
        {
  ...
  reducers: {
    ...
    removeCategoryFromWords: (state, action: PayloadAction<string>) => {
      state.values.forEach((word) => {
        word.categories = word.categories.filter(
    ...
    addRecentlyLearntWord: (state, action: PayloadAction<WordOrPhrase>) => {
      ...
      state.recentlyLearntWordsAndPhrases =
        state.recentlyLearntWordsAndPhrases.filter(
    ...
    setupWordToLearn: (state, action: PayloadAction<string[]>) => {
      const wordsInList =
        !!action.payload && action.payload.length > 0
          ? state.values.filter((word) => action.payload.includes(word.id))
          : state.values;
      const wordToLearn = WordHelper.getRandomWordOrPhraseWithLowestLearnLevel(
        wordsInList,
        [],
        [],
        state.recentlyLearntWordsAndPhrases,
      );
      const choice1 = WordHelper.getRandomWordOrPhrase(
        state.values,
        wordToLearn.categories,
        [wordToLearn.id],
        state.recentlyLearntWordsAndPhrases,
      );
      const choice2 = WordHelper.getRandomWordOrPhrase(
        state.values,
        wordToLearn.categories,
        [wordToLearn.id, choice1.id],
        state.recentlyLearntWordsAndPhrases,
      );
      const learn = {
    ...
    setWordLearntAsCorrect: (state) => {
      if (state.learn?.wordToLearn) {
        const word = state.values.find(
          (wp) => wp.id === state.learn?.wordToLearn.id,
        );
    ...
    setWordEditReturnRoute: (
      state,
      action: PayloadAction<{ route: string; routeId?: string }>,
    ) => {
...
export const {
  ...
  updateWord,
  removeWord,
  removeCategoryFromWords,
  search,
  loadWords,
  addRecentlyLearntWord,
  loadRecentlyLearntWordsAndPhrases,
  setupWordToLearn,
  ...
  setWhatToLearnListId,
  setWordEditReturnRoute,
} = wordsAndPhrasesSlice.actions;
// Export the slice reducer for use in the store configuration
export default wordsAndPhrasesSlice.reducer;

export const selectAllWordsAndPhrases = (state: RootState) =>
  state.wordsAndPhrases.values;
export const selectSearchQuery = (state: RootState) =>
  state.wordsAndPhrases.searchQuery;
export const selectRecentlyLearntWordsAndPhrases = (state: RootState) =>
  state.wordsAndPhrases.recentlyLearntWordsAndPhrases;
export const selectLearn = (state: RootState) => state.wordsAndPhrases.learn;
export const selectWhatToLearnListId = (state: RootState) =>
  state.wordsAndPhrases.whatToLearnListId;
export const selectWordEditReturnRoute = (state: RootState) =>
  state.wordsAndPhrases.wordEditReturnRoute;

export const selectWordsBySearchQuery = createSelector(
  [selectAllWordsAndPhrases, selectSearchQuery],
  (words, query) => {
    return query
      ? words.filter(
          (word) =>
...
export const selectWordById = createSelector(
  [selectAllWordsAndPhrases, selectWordId],
  (words, id) => {
    return id ? words.find((word) => word?.id === id) : null;
  },
);

const selectWordList = (
  state: RootState,
  wordList: WordList | null | undefined,
) => wordList;
export const selectWordsForList = createSelector(
  [selectAllWordsAndPhrases, selectWordList],
  (words, wordList) => {
    return wordList?.items
      ? wordList.items.map((wordListItem) => ({
          actor: wordListItem.actor,
          wordOrPhraseId: wordListItem.wordOrPhraseId,
          wordOrPhrase: words.find(
            (wp) => wp.id === wordListItem.wordOrPhraseId,
          ) || {
            id: "",
            value: "",
            soundsLike: "",
...
export const selectWordsByCategoryId = createSelector(
  [selectAllWordsAndPhrases, selectCategoryId],
  (words, id) => {
    return id ? words.filter((word) => word?.categories?.includes(id)) : [];
  },
);
